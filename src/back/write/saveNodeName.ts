...
import {buildLocalizedString, parseNodeNamesAndLanguageCode, escapeRegexp, getTaxonomyGraphFilePath, getTaxonomyFilePath} from '../utils';
...
type ParentNamesWithLanguageCodeType = GraphNodeType['parentNamesWithLanguageCode'];
...
const updateNodeName = async(nodeNameStringWithLanguageCode: string, newLanguageCode: string, newNodeNames: string[], txtFile: string[], graphJson: GraphNodeType[],index=0, parentNamesWithLanguageCode: ParentNamesWithLanguageCodeType) => {
    
    const nodeNameRegexp = new RegExp(`^${escapeRegexp(nodeNameStringWithLanguageCode)}`)
    ...
    const {nodeNamesLanguageCode: originalLanguageCode, nodeNames: originalNodeNames} = parseNodeNamesAndLanguageCode(originalNode.nodeNameStringWithLanguageCode)


    // check if is the same as original
    const newNodeNamesLowerCaseSet = new Set(newNodeNames.map((nodeNameItem) => nodeNameItem.toLowerCase())); // values are case insensitive
    if (originalLanguageCode === newLanguageCode && 
        originalNodeNames.length === newNodeNames.length
         && originalNodeNames.every((nameItem) =>  newNodeNamesLowerCaseSet.has(nameItem.toLowerCase()))) {
    ...
    for (const nodeItem of graphJson) {
        const {nodeNames: nodeItemNames} = parseNodeNamesAndLanguageCode(nodeItem.nodeNameStringWithLanguageCode);

        if (nodeItem.nodeNameStringWithLanguageCode ===  nodeNameStringWithLanguageCode) continue;

        if (nodeItemNames.some( nameItem => newNodeNamesLowerCaseSet.has(nameItem.toLowerCase()))) {
            return Promise.reject(buildError({code: UpdateNodeNameErrorCodes.invalidNewNames, message:`Unable to save new name. Other nodes already share some synonyms with it.`})  )
        }
    }
    ...
    const {txtFile: updatedNodeTxtFile, graphJson: updatedNodeGraphJson} = updateFiles({
    ...
    // update json
    const originalNodeNamesSet = new Set(originalNodeNames.map(nameItem => nameItem.toLowerCase()));
    ...
    const updatedParentsGraphJson = updatedNodeGraphJson.map((nodeItem) => {
        if (nodeItem.parentNamesWithLanguageCode.some(parentNameItem =>  parentNameItem === nodeNameStringWithLanguageCode)) {
            
            const newParentsWithLanguageCode = nodeItem.parentNamesWithLanguageCode
                .map(parentNameItem => parentNameItem === nodeNameStringWithLanguageCode? newNodeNameString: parentNameItem );
    ...
    const updatedParentsTxt = updatedNodeTxtFile.map(rowItem => {
        ...
        if (rowInfo.startsWith(characters.childrenOf)) {
            ...
            if (originalNodeNamesSet.has(parentName)) {
                return buildLocalizedString( newLanguageCode, [defaultNewParentName],true, isCommentRow)
            }
        } 
...
const saveNodeName = async( {taxonomy, nodeNameStringWithLanguageCode, newLanguageCode, newNodeNames, index, parentNamesWithLanguageCode}:SaveNodeNameArgs) => {
    ...
    if (!ISO6391.validate(newLanguageCode)) return Promise.reject( buildError({code: UpdateNodeNameErrorCodes.invalidLanguageCode, message:`Unable to save new name. Invalid language code.`}) )
    if (newNodeNames.length === 0) return Promise.reject(buildError({code: UpdateNodeNameErrorCodes.invalidNewNames, message:`Unable to save new name. Invalid value`})  )



    const {txtFile: updatedTxtFile, graphJson: updatedGraphJson} = await updateNodeName(nodeNameStringWithLanguageCode, newLanguageCode, newNodeNames, taxonomyFile, graphJson,index, parentNamesWithLanguageCode)
