...
type saveNodeInfo = Pick<
  GraphNodeType,
  "parentNamesWithLanguageCode" | "languages" | "comments" | "otherInfo"
>;
...
const updateNode = (
  ...
  const languagesWithValues = Object.keys(newNode.languages)
    .map((keyItem) => {
      const languageLength = newNode.languages[keyItem].value
  ...
  Object.keys(newNode.languages).forEach((keyItem) => {
    if (!languagesWithValues.includes(keyItem)) {
      ...
      newNode.languages[keyItem].value = newNode.languages[keyItem].value.map(
  ...
  const cleanedComments = newNode.comments
    .map((commentItem) => commentItem.trim())
    .filter(Boolean);

  // clean other info pieces. They should at least contain the first two pieces of information
  const cleanedOtherInfo = newNode.otherInfo.filter(
...
const saveUpdateNode = async ({
  ...
  const { txtFile: updatedTxtFile, graphJson: updatedGraphJson } = updateNode(
    nodeNameStringWithLanguageCode,
    taxonomyFile,
    graphJson,
    index,
