...
const getNode = async({nodeName}:GetNodeArgs) => {
    ...
    try{
        graphData = JSON.parse(fs.readFileSync(graphFilePath).toString()) as GraphNodeType[];
    } catch(error) {
        throw Promise.reject(new Error(`Unable to read the graph file, please try again`))
    }
    
    if (!graphData || graphData.length === 0) throw Promise.reject(`No data in the taxonomy, please check`)
    ...
    targetNodes.forEach((targetNodeItem) => {
        ...
        const targetParents = getParentsTree({nodeNameWithLanguageCode:nodeNameStringWithLanguageCode, includeTarget:false, searchType:'all', graphData});
        const targetChildren = getChildrenTree({nodeNameWithLanguageCode:nodeNameStringWithLanguageCode, includeTarget:false, searchType:'oneLevelDown', graphData});
