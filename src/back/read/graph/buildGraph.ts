...
const parseAndCleanTaxonomy = (taxonomy: TaxonomyType) => {
  ...
  const getPrevLocatedKey = (parentName: string, parentLanguageCode: string) =>
    `parent-${parentName}-lang-${parentLanguageCode}`;

  nodes.forEach((nodeItem) => {
    const parents = nodeItem.parents.filter(
      (parentItem) => !parentItem.isDeprecated
    );
    ...
    parents.forEach((parentItem) => {
      ...
      if (prevKey in prevLocated) {
        ...
        for (let i = 0; i < nodes.length; i++) {
          const { nodeNamesLanguageCode, nodeNames } =
            parseNodeNamesAndLanguageCode(
              nodes[i].nodeNameStringWithLanguageCode
            );
          const currentNodeNames = nodeNames.map((nameItem) =>
            nameItem.toLowerCase()
          );
          const currentNodeNamesLanguageCode = nodeNamesLanguageCode;
          const currentNodeNameStringWithLanguageCode =
            nodes[i].nodeNameStringWithLanguageCode;
          ...
          if (
            currentNodeNamesLanguageCode === parentLanguageCode &&
            currentNodeNames.includes(parentName.toLowerCase())
          ) {
            parentsNodesNamesWithLanguageCode.push(
              currentNodeNameStringWithLanguageCode
            );
...
const buildGraph = async ({ taxonomy, forceSave }: BuildGraphArgs) => {
  ...
  if (!fs.existsSync(taxonomyFilePath)) {
    throw Promise.reject(
      new Error(`Unable to find taxonomy (${taxonomy}) file`)
    );
  }
  ...
  fs.writeFileSync(filePath, JSON.stringify(nodesJson));
