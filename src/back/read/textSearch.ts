...
import {getTaxonomyFilePath, parseNodeNamesAndLanguageCode, escapeRegexp} from '../utils';
...
const textSearch = async({searchText, taxonomy}:TextSearchArgs) => {
    ...
    if (searchText.length <= 2) {
        return Promise.reject(new Error(`Please search a bigger string: ${searchText}`))
    }
    ...
    if (!fs.existsSync(taxonomyFilePath)) {
        return Promise.reject(new Error(`Unable to find taxonomy file: ${taxonomy}`))
    }
    ...
    try{
        ...
        console.error(error);
        return Promise.reject(new Error(`Unable to read taxonomy file: ${taxonomy}`))
    }
    ...
    for(const {startRow, row, endRow} of searchResult) {
        ...
        if (node) {
            const nodeNameStringWithLanguageCode = node.nodeNameStringWithLanguageCode  ;

            const {nodeNames, nodeNamesLanguageCode} = parseNodeNamesAndLanguageCode(nodeNameStringWithLanguageCode);
            const nodeName:string|null = nodeNames?.length > 0?nodeNames.join(characters.taxonomySeparator):null;

            results.push({nodeNameStringWithLanguageCode, nodeName, nodeNamesLanguageCode, text:rows, fileRows: {startRow, row, endRow}})

        } else {

            results.push({nodeNameStringWithLanguageCode:null, nodeName:null, nodeNamesLanguageCode: null, text:rows, fileRows: {startRow, row, endRow}})
        }
