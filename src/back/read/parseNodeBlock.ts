...
const parseNodeBlock = ({
  ...
  if (!nodeNameStringWithLanguageCode) {
    console.error(
      `Unable to find node name. Start line: ${startRow}, end line: ${endRow}`
    );
    return null;
  }
  ...
  for (let i = startRow; i <= endRow; i++) {
    ...
    if (prefix.startsWith(characters.childrenOf)) {
      ...
      const [languageCode, value] = row
        .replace(characters.childrenOf, "")
        .split(characters.propsSeparator);
    ...
    if (languageCodesSet.has(prefix) && prefix !== nodeNameLanguageCode) {
      ...
      languages[prefix] = {
        isDeprecated: false,
        value: languageString.split(characters.taxonomySeparator),
      };
    }

    if (
      !prefix.startsWith(characters.childrenOf) &&
      !prefix.startsWith("#") &&
      !languageCodesSet.has(prefix)
    ) {
      // string looks like:
      // 'wikidata:en:Q217446'
      ...
      const [property, languageCode, ...rest] = row.split(
        characters.propsSeparator
      );
      otherInfo.push({
        property,
        languageCode,
        value: rest.join(characters.propsSeparator),
        isDeprecated: false,
      });
    }
    ...
    if (prefix.startsWith(characters.comment)) {
      ...
      if (languageCodesSet.has(commentPrefix)) {
        ...
        languages[commentPrefix] = {
          isDeprecated: true,
          value: cleanedRow
            .split(characters.propsSeparator)[1]
            .split(characters.taxonomySeparator),
        };
      } else if (commentPrefix.startsWith(characters.childrenOf)) {
        // strings starting with '<' are children of other nodes
        //  examples: '<en:Confectioneries', '<en:French confectioneries'
        const [languageCode, value] = cleanedRow
          .replace(characters.childrenOf, "")
          .split(characters.propsSeparator);
        ...
        const [property, languageCode, value] = cleanedRow.split(
          characters.propsSeparator
        );
