...
type ContextDispatch = React.Dispatch<
  React.SetStateAction<TaxonomyContextType>
>;

const TaxonomyContext = createContext<{
  context: TaxonomyContextType;
  setContext: ContextDispatch;
}>(null);
export const useTaxonomyContext = () => {
  ...
  if (!value) {
    throw new Error(
      "Please use useTaxonomyContext inside its context provider"
    );
  }
...
const Taxonomy = () => {
  ...
  const [taxonomyContextState, setTaxonomyContextState] =
    useState<TaxonomyContextType>({ taxonomy: null, languageCode: "en" }); // TODO fix this, this is duplicating code
  ...
  useEffect(function getLanguageCode() {
    // TODO promisify type
    getSettings({ key: "languageCode" }).then((res) =>
      setTaxonomyContextState((prev) => ({ ...prev, languageCode: res }))
    );
  }, []);
  ...
  useEffect(
    function createGraph() {
      ...
      setTaxonomyContextState((prev) => ({
        ...prev,
        taxonomy: castedTaxonomy,
      }));
      ...
      const downloadAndBuildGraph = async () => {
        ...
        try {
          await downloadFile({
            taxonomy: castedTaxonomy,
            gitForkUrl: "nobeeakon/openfoodfacts-server",
            forceDownload: false,
          });
  ...
  if (status === "error")
    return (
      <div>
        Failed to download the taxonomy
        ...
        <>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <div>
              ...
              <select
                value={taxonomyContextState?.languageCode}
                id="language-select"
                onChange={onLanguageChange}
              >
                {ISO6391.getAllNames().map((nameItem) => (
                  <option key={nameItem} value={ISO6391.getCode(nameItem)}>
                    {nameItem}
                  </option>
                ))}
              </select>
          ...
          <TaxonomyContext.Provider
            value={{
              context: taxonomyContextState,
              setContext: setTaxonomyContextState,
            }}
          >
            <Outlet />
          </TaxonomyContext.Provider>
