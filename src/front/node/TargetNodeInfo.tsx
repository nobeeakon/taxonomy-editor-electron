...
const NodeInfo = ({ node, childrenNode, index }: Props) => {
  ...
  const childrenNames = (childrenNode || [])
    .filter((childItem) => {
      const { parentNamesWithLanguageCode } = childItem;
      return parentNamesWithLanguageCode.includes(
        nodeNameStringWithLanguageCode
      );
    })
    .map((childItem) => childItem.nodeNameStringWithLanguageCode)
    .sort();

  const langsToShow = languages[context.languageCode];
  const childrenNamesToShow = !childrenSearchTerm
    ? childrenNames
    : childrenNames.filter((childItem) =>
        childItem.toLowerCase().includes(childrenSearchTerm.toLowerCase())
      );
  const parentsNamesToShow = !parentsSearchTerm
    ? [...parentNamesWithLanguageCode].sort()
    : [...parentNamesWithLanguageCode]
        .sort()
        .filter((parentItem) =>
          parentItem.toLowerCase().includes(parentsSearchTerm.toLowerCase())
        );

  const nodeUpdate = <Key extends keyof NodeResultType["node"]>(
    key: Key,
    newValue: NodeResultType["node"][Key]
  ) => {
    setUpdatedNode((prev) => {
      const newNode = { ...prev };
  ...
  const onSaveNodeInformation = () => {
    ...
    saveNodeInfo({
      ...
      .catch(() => console.log("fracaso ,......")); // FIXME TODO remove
  };
  ...
  return (
    <>
        ...
        <div>
          ...
          <NodeName
            ...
            updateNodeName={(newNodeName) =>
              nodeUpdate("nodeNameStringWithLanguageCode", newNodeName)
            }
            index={index}
      ...
      <div>
        ...
        {langsToShow?.value.map((synonymItem, index) => (
          <input
            key={index}
            value={synonymItem}
            onChange={(event) =>
              onChangeLocalization(event.target.value, index)
            }
          />
        ))}

        <button
          onClick={() => onChangeLocalization("", langsToShow.value.length)}
        >
          Add synonym
        </button>
      </div>
      ...
      <div>
        ...
        <NodeComments
          comments={comments}
          updateComments={(newComments) => nodeUpdate("comments", newComments)}
        />
      </div>
      <div>
        <h3>Other info </h3>
        ...
        <NodeOtherInfo
          nodeOtherInfo={otherInfo}
          updateNodeOtherInfo={(newValue) => nodeUpdate("otherInfo", newValue)}
        />
      </div>
      <div></div>
      <div>
        {hasChanged && <button onClick={onSaveNodeInformation}>Save</button>}
      </div>
      <div>
        {parentNamesWithLanguageCode.length > 0 && (
          <>
              ...
              {parentsNamesToShow.length > 4 && (
                <>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setParentsSearchTerm(event.target.value)
                    }
                  />
                </>
              )}
            ...
            <div>
              {parentsNamesToShow.map((parentItem) => (
                <div>
                  <Link to={encodeURI(`/${context.taxonomy}/${parentItem}`)}>
                    {parentItem}
                  </Link>
                </div>
              ))}
        ...
        {childrenNames.length > 0 && (
          <>
            ...
            <div>
              {childrenNamesToShow.length > 4 && (
                <>
                  ...
                  <input
                    type="text"
                    id="search-children-input"
                    value={childrenSearchTerm}
                    onChange={(event) =>
                      setChildrenSearchTerm(event.target.value)
                    }
                  />
                </>
              )}
            ...
            <div>
              {childrenNamesToShow.map((childItem) => (
                <div>
                  <Link to={encodeURI(`/${context.taxonomy}/${childItem}`)}>
                    {childItem}
                  </Link>
                </div>
              ))}
