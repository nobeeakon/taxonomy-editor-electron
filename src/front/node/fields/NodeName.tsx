...
const EditNodeName = ({
  ...
  const [editNodeName, setEditNodeName] = useState(() => {
    const [languageCode, nodeNamesString] =
  ...
  const onUpdateNodeName = () => {
    if (!validateNewName(editNodeName.languageCode, editNodeName.nodeNames))
      return;

    const filteredNodeNames = editNodeName.nodeNames
      .map((nameItem) => nameItem.trim())
      .filter(Boolean);
  ...
  return (
    <div>
      <div>
        <select
          value={editNodeName.languageCode}
        ...
        {editNodeName.nodeNames.map((nameItem, nameIndex) => (
          <input
            onChange={(event) => onChangeNames(event.target.value, nameIndex)}
            value={nameItem}
            type="text"
          />
        ))}
        <button
          onClick={() => onChangeNames("", editNodeName.nodeNames.length)}
        >
          Add synonym
        </button>
      </div>
      <div>
        <button
          ...
          disabled={
            !validateNewName(editNodeName.languageCode, editNodeName.nodeNames)
          }
        >
          Save
        </button>
...
const NodeName = ({
  ...
  const onSaveNodeName = (newLanguageCode: string, newNodeNames: string[]) => {
    const newNodeNameWithLanguageCodeString = `${newLanguageCode}${
      characters.propsSeparator
    }${newNodeNames.join(characters.taxonomySeparator)}`;
  ...
  if (!isEditNodeName)
    return (
      <>
        {" "}
        {nodeNameStringWithLanguageCode}{" "}
        <button onClick={() => setIsEditNodeName(true)}>Edit</button>
      </>
    );
