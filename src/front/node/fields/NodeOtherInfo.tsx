...
const NodeOtherInfo = ({nodeOtherInfo, updateNodeOtherInfo}: Props) => {
    ...
    const onOtherInfoChange = (index: number, newValue:string, type: 'property'|'languageCode'|'value') => {
    ...
    const onAddOtherInfo = () => {
        ...
        newOtherInfo.push({property:'', languageCode:'', value:'', isDeprecated: false})
    ...
    return  <div>
        ...
        <div> <i>NOTE: May contain invalid language codes. We are using: ISO 639-1 </i></div>
    <table style={{border:'1px solid grey', margin: '2rem'}}>
        ...
        <tbody>
            {nodeOtherInfo.map(({property, languageCode, value}, index) => <tr key={`${property}-${languageCode}-${value}`}>
                <td>  <input type="text" value={property}
                 onChange={(event) => onOtherInfoChange(index, event.target.value, 'property')}
                />         </td>
                <td>{!languageCode || ISO6391.validate(languageCode)?<select value={languageCode}
                 onChange={(event) => onOtherInfoChange(index, event.target.value, 'languageCode')}
                >
                    ...
                    <input type="text" value={languageCode}
                    onChange={(event) => onOtherInfoChange(index, event.target.value, 'languageCode')}
                    /> 
                }
                </td>
                <td><input type="text" value={value} onChange={(event) => onOtherInfoChange(index, event.target.value, 'value')}/></td>
                <td><button onClick={() => onRemoveOtherInfo(index)}>remove</button></td>
            </tr>)}
        </tbody>
    </table>
