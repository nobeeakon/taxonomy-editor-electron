...
const NodeInfo = () => {
  ...
  return (
    <>
      ...
      <div>
        {nodeInfo.length === 0 && status === "success" && (
          <div> No node found </div>
        )}
        {nodeInfo.length > 1 && status === "success" && (
          <div>
            {" "}
            Found: {nodeInfo.length} nodes. Please check if these are duplicates{" "}
          </div>
        )}
        {nodeInfo.length > 0 && status === "success" && (
          <div>
            {nodeInfo.map((nodeItem, index) => (
              <div
                key={`${index}-${nodeItem.node.nodeNameStringWithLanguageCode}`}
              >
                <TargetNodeInfo
                  node={nodeItem.node}
                  childrenNode={nodeItem.children}
                  index={index}
                />

                <CytoscapeComponent
                  elements={getCytoscape(nodeItem)}
